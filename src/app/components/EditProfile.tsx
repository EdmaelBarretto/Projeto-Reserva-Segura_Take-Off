...
import { getCurrentLevel } from "../utils/xp";
...
export default function EditProfile({ onBack }: EditProfileProps) {
  ...
  const nivelAtual = getCurrentLevel(user?.xpTotal || 0);
  ...
  useEffect(() => {
    ...
    userService.buscar(user.userId)
      .then((data: any) => {
        setName(data?.nome ?? data?.name ?? user?.nome ?? "");
        setEmail(data?.email ?? user?.email ?? "");
      })
      .catch(console.error);
  }, [user?.userId, user?.nome, user?.email]);
  return (
    <div className="fixed inset-0 bg-white z-50 overflow-auto">
      ...
      <div className="p-4 pb-20">
        ...
        <div className="flex flex-col items-center mb-8 py-4">
          ...
          <h2 className="text-xl font-bold text-[#101828] mb-1">{userName}</h2>
          <p className="text-sm text-[#7e848d]">Aspirante Nível {nivelAtual}</p>
        </div>
